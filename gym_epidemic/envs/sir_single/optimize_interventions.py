...
def I_of_S(S, R0, S0=1, I0=0):
    return I0 + S0 - (1 / R0) * np.log(S0) - S + (1 / R0) * np.log(S)
...
def S_of_I(I, R0, S0=1, I0=0):
    ...
    if I > Ipeak:
        raise ValueError("I must be smaller " "than I_peak = {}".format(Ipeak))
    exponent = R0 * (I - I0 - S0)
...
def S_of_t(t, R0, gamma, fineness=100000, I0=1e-6):
    ...
    def deriv(state, time):
        S, I = state
        return np.array([-R0 * gamma * S * I, R0 * gamma * S * I - gamma * I])

    return odeint(deriv, [1 - I0, I0], times)[-1:, 0]


def I_max(S, I, R0):
    return S + I - (1 / R0) * np.log(S) - (1 / R0) + (1 / R0) * np.log(1 / R0)
...
def I_max_opt_of_S_i(S_i, f, R0, gammatau):
    I_i = I_of_S(S_i, R0)
    return max(I_max_after_opt_intervention(S_i, I_i, f, R0, gammatau), I_i)
...
def calc_Sf_opt(R0, gammatau, method=None, func=log_Imax_vec):
    S_target = 1
    ...
    minima = minimize(
        ...
        method=method,
        constraints=[
            {"fun": constrain_Scrit, "type": "ineq", "args": (R0, gammatau)},
            {"fun": constrain_I_i, "type": "eq", "args": (R0, gammatau)},
        ],
        bounds=([0, 1], [0, 1]),
    )
    if minima.success:
...
def calc_Sf_opt_brute(R0, gammatau, f_guess_init=0.5, n_refinements=5):

    f_guess = f_guess_init
    f_tol = 1
    for i_ref in range(n_refinements):
        fs = np.linspace(max(0, f_guess - f_tol), min(1, f_guess + f_tol), 100)
        vals = [
            I_max_opt_of_S_i(calc_S_var_opt(R0, gammatau, f), f, R0, gammatau)
            for f in fs
        ]
...
def calc_f_opt(R0, gammatau, gamma=None, S_i=None, t_i=None, method="bounded"):
    if t_i is not None and gamma is not None:
        S_i = S_of_t(S, R0, gamma)
    elif S_i is None:
        raise ValueError("Must provide either S_i or " "t_i and gamma")
    I_i = I_of_S(S_i, R0)
    if S_i <= 1 / R0:
        raise ValueError("S must be greater than " "Scrit = 1/R0")
    max_f = (S_i - 1 / R0) / (gammatau * I_i)
    minima = minimize_scalar(
        log_Imax_opt,
...
def min_S(R0, f, gammatau):
    ...
    Rfgo = R0 * (fg + 1)
    candidate = np.real(
        -fg * lambertw(-np.exp(-(1 / fg) - R0) * Rfgo / fg, k=-1) / Rfgo
    )
    return max(1 / R0, candidate)


def calc_S_var_opt(R0, gammatau, f, method="bounded"):
...
def Imax_of_S_i_b(S_i, b, R0, gamma, tau):
    I_i = I_of_S(S_i, R0)
    ...
    intervention = odeint(
        deriv, [S_i, I_i], np.linspace(0, tau, (1 + int(tau)) * 1000)
    )
...
def calc_Sb_opt(
    ...
    def func(Sb_vec):
        S_i, b = Sb_vec
        return Imax_of_S_i_b(S_i, b, R0, gamma, tau)

    Imax = 1
    result = np.array([np.nan, np.nan])
    for k in range(n_max_tries):
        b_guess = max(0, min(np.random.normal(b_guess, 0.05), 1))
        guess_vec = np.array([S_guess, b_guess])
        minima = minimize(
            func, guess_vec, method=method, bounds=([1 / R0, 1], [0, 1])
        )
        if minima.success:
            if minima.fun < Imax:
                result = minima.x
