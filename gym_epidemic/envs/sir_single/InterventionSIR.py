...
class InterventionSIR:
    ...
    def __init__(self, b_func=None, R0=None, gamma=None, inits=None):
        if b_func is None:
            b_func = Intervention()
        self.b_func = b_func
        self.R0 = R0
    ...
    def integrate(self, final_time, fineness=10000):
        times = np.linspace(self.time, final_time, fineness)
        results = odeint(self.deriv, self.state, times)
        self.state = results[-1]
        self.time = final_time
        self.time_ts = np.concatenate([self.time_ts, times])
        self.state_ts = np.concatenate([self.state_ts, results])
    ...
    def get_I_max(self, allow_boundary_max=False):
        last_timestep_error = (
    ...
    def __repr__(self):
        return (
            "InterventionSIR with R0 = {}, "
            "gamma = {}, and an intervention "
            "function {}".format(self.R0, self.gamma, self.b_func)
        )
...
class Intervention:
    ...
    def maintain_contain_state(self, time, beta, gamma, S, I):
        """
        ...
        if time >= self.t_i and time < self.t_i + self.tau * self.f:
            ...
            time >= self.t_i + self.tau * self.f and time < self.t_i + self.tau
        ):
            result = self.sigma_1
        else:
            result = 1
...
def make_fixed_b_func(tau, t_i, sigma):
    ...
    """
    return Intervention(tau=tau, t_i=t_i, sigma=sigma, strategy="fixed")
