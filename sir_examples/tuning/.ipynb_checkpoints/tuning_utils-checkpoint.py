...
def hyperparam_optimization(algo, model_fn, env_fn, n_trials=10, n_timesteps=30000, hyperparams=None,
    ...
    def objective(trial):
        ...
        if algo in ['ddpg', 'td3'] or trial.model_class in ['ddpg', 'td3']:
            trial.n_actions = env_fn().action_space.shape[0]
        ...
        try:
            model.learn(n_timesteps, callback=eval_callback, log_interval=1000)
            # Free memory
            model.env.close()
            eval_env.close()
